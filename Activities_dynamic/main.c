...
int main(){
    ...
    struct activities *v; // linking the vector "v" to the activities structure


    printf("Input the number of activities:");
    scanf("%d", &n); // input the number of activities

    v = malloc(n * sizeof(struct activities)); // dynamic allocation for the vector "v"
    ...
    select_activities(v, n); // call for the function "select_activity"
    return 0;
}
